...
function clearTheme() {
  let now = new Date();
  document.cookie = `themeName=;expires=${now.toString()}`
...
function restoreName() {
  let displayName = document.cookie
    .split('; ')
    .find(cookie => cookie.startsWith('displayName'))
    ?.split('=')[1];
...
function clearName() {
  let now = new Date();
  document.cookie = `displayName=;expires${now.toString()}`
}
/* ========================================================================= */
